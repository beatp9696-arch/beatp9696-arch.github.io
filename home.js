import { escapeHTML as esc, dateLabel, needsReview } from './pp-os/js/core/research-model.js';
...
function render() {
  ...
  host.innerHTML = filtered.length ? filtered.map(c => `<a class="home-company" href="pp-os/?mode=app&amp;tab=moatrices&amp;company=${encodeURIComponent(c.ticker)}">
    <span class="home-company-logo"><span>${esc(c.ticker.slice(0, 2))}</span><img src="pp-os/assets/brands/${encodeURIComponent(c.ticker)}.png" alt="" width="42" height="42"></span>
...
async function start() {
  ...
  try {
    const response = await fetch('./pp-os/data/research.json');
