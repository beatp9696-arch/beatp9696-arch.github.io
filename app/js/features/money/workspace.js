import {load,updateStorage,watchStorage,onDataChange} from '../../core/storage.js';
...
const storageDefaults=Object.fromEntries(Object.entries(defaults).map(([key,value])=>['money.'+key,value]));
const read=()=>Object.fromEntries(Object.entries(defaults).map(([key,value])=>[key,structuredClone(load('money.'+key,value))]));
function unchanged(current,previous,label) {
  if(!current)throw new Error(`This ${label} was removed elsewhere. Close this editor and check your data.`);
  if(JSON.stringify(current)!==JSON.stringify(previous))throw new Error(`This ${label} changed in another tab. Close this editor and reopen it before saving.`);
}
...
export function mountMoney(body) {
  ...
  let saving=false,activeDialog=null,renderQueued=false,renderPending=false;
  const scrollHost=()=>body.closest('#shell-view')||body;
  function render(focus) {
    if (!body.isConnected) return;
    if (activeDialog?.open) {renderPending=true;return;}
    renderPending=false;
  ...
  function dialog(title,content,trigger) {
    ...
    el.addEventListener('close',()=>{el.remove();if(activeDialog===el)activeDialog=null;if(renderPending)render();if(trigger?.isConnected)trigger.focus({preventScroll:true});else body.querySelector('.mn-head-actions [data-action="add"]')?.focus({preventScroll:true});});
  ...
  async function persist(update) {
    saving=true;
    try {
      await updateStorage(storageDefaults,stored=>{
        const latest=Object.fromEntries(Object.keys(defaults).map(key=>[key,stored['money.'+key]]));
        return Object.fromEntries(Object.entries(update(latest)).map(([key,value])=>['money.'+key,value]));
      });
  ...
  function editEntry(id,trigger,preset={}) {
    ...
    el.querySelector('[data-action="delete-dialog"]')?.addEventListener('click',()=>{el.close();confirmDelete('Delete this transaction?','The entry will be removed and your recorded balance and allocations recalculated.',async()=>{await persist(latest=>{unchanged(latest.entries.find(e=>e.id===previous.id),previous,'transaction');return {entries:latest.entries.filter(e=>e.id!==previous.id)};});},trigger);});
    submit(el,async values=>{
      await persist(latest=>{
        if(previous)unchanged(latest.entries.find(e=>e.id===previous.id),previous,'transaction');
        ...
        return {entries:[...latest.entries.filter(e=>e.id!==entry.id),entry]};
      });finish(el,'Transaction saved.');
    });
  }
  function addTransfer(trigger) {
    ...
    submit(el,async values=>{
      ...
      const transfer={id:crypto.randomUUID(),amount,date:values.date,note};
      await persist(latest=>({transfers:[...latest.transfers,transfer]}));
  ...
  function editBudget(cat,trigger) {
    ...
    el.querySelector('[data-action="delete-dialog"]')?.addEventListener('click',async()=>{
      if(saving)return;
      try {await persist(latest=>{delete latest.budgets[cat];return {budgets:latest.budgets};});finish(el,'Budget limit removed. Transactions are still in your ledger.');}catch(error){formError(el,error.message);}
    });
    submit(el,async values=>{const name=cat||values.cat.trim();if(!name||['__proto__','constructor','prototype'].includes(name))throw new Error('Choose a category name.');await persist(latest=>({budgets:{...latest.budgets,[name]:validateAmount(values.amount)}}));finish(el,'Monthly budget saved.');});
  }
  function editGoal(id,trigger) {
    ...
    el.querySelector('[data-action="delete-dialog"]')?.addEventListener('click',()=>{el.close();confirmDelete('Delete this goal?',`${money(old.saved)} will become unassigned savings. Your total balance stays the same.`,async()=>{await persist(latest=>({goals:latest.goals.filter(g=>g.id!==old.id)}));},trigger);});
    submit(el,async values=>{
      const name=values.name.trim();if(!name)throw new Error('Give your goal a name.');
      await persist(latest=>{
        const goals=latest.goals,current=goals.find(g=>g.id===goalId);
        if(old)unchanged(current,old,'goal');
        const goal={...current,id:goalId,name,target:validateAmount(values.amount),saved:current?.saved||0,emoji:current?.emoji||'🎯'};
        return {goals:[...goals.filter(g=>g.id!==goalId),goal]};
      });finish(el,'Savings goal saved.');
    });
  }
  function manageFunds(id,trigger) {
    ...
    submit(el,async values=>{await persist(latest=>{const current=latest.goals.find(g=>g.id===goal.id);if(!current)throw new Error('This goal no longer exists.');const updated=fundGoal(current,values.amount,values.direction,balances(latest.entries).savings-allocatedToGoals(latest.goals));return {goals:latest.goals.map(g=>g.id===goal.id?updated:g)};});finish(el,'Goal funds updated.');});
  }
  function settings(trigger) {
    ...
    submit(el,async values=>{await persist(latest=>({split:validateSplit({savings:Number(values.savings),invest:Number(values.invest)}),card:{...latest.card,roundups:values.roundups==='on'}}));finish(el,'Money settings saved. Existing income allocations are unchanged.');});
  }
  function editBill(id,trigger) {
    ...
    el.querySelector('[data-action="delete-dialog"]')?.addEventListener('click',()=>{el.close();confirmDelete('Delete this reminder?','Future reminders will be removed. Payments already recorded remain in your transactions.',async()=>{await persist(latest=>({recurring:latest.recurring.filter(b=>b.id!==old.id)}));},trigger);});
    submit(el,async values=>{const name=values.name.trim(),day=Number(values.day);if(!name||!Number.isInteger(day)||day<1||day>31||!validDate(values.startDate))throw new Error('Enter a name, a due day from 1 to 31, and a valid start date.');await persist(latest=>{if(old)unchanged(latest.recurring.find(b=>b.id===billId),old,'reminder');return {recurring:[...latest.recurring.filter(b=>b.id!==billId),{...old,id:billId,name,amount:validateAmount(values.amount),day,startDate:values.startDate,cat:values.cat}]};});finish(el,'Monthly bill reminder saved.');});
  }
  async function previewImport(file,trigger) {
    ...
    try {
      ...
      submit(el,async values=>{let count=0;await persist(latest=>{const known=new Set(latest.entries.map(entryFingerprint));const toAdd=entries.filter(e=>!latest.entries.some(old=>old.id===e.id)&&(!values.skip||!known.has(entryFingerprint(e))));count=toAdd.length;return {entries:[...latest.entries,...toAdd]};});finish(el,`${count} transaction${count===1?'':'s'} imported.`);});
  ...
  body.addEventListener('click',async event=>{
    ...
    try {
      ...
      if(action==='toggle-review'){await persist(latest=>({entries:latest.entries.map(e=>String(e.id)===id?{...e,reviewed:!e.reviewed}:e)}));render(`[data-entry="${CSS.escape(id)}"] [data-action="toggle-review"]`);return;}
  ...
  const unwatch=watchStorage(Object.keys(storageDefaults));
  const detector=new MutationObserver(()=>{if(!body.isConnected){life.abort();unsubscribe();unwatch();detector.disconnect();}});
