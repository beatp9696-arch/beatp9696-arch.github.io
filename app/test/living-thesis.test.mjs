...
// เดิมเช็คลิสต์ precache ของ sw.js ว่าไฟล์มีจริง — แอปเป็นหน้าเว็บแล้ว ไม่มี SW ของตัวเอง
// ตัวที่พังเงียบแทนคือ import ที่ชี้ผิดไฟล์ กับ stylesheet ที่ลืมผูกเข้า portfolio.html
const page=readFileSync(new URL('../../portfolio.html',import.meta.url),'utf8');
assert.ok(page.includes('app/css/living-thesis.css'),'portfolio.html does not load living-thesis.css');
const modules=['../js/apps/portfolio.js','../js/apps/portfolio-allocation.js',
 ...['model','service','store','components','overview','detail','workspace'].map(n=>`../js/features/living-thesis/${n}.js`)];
for(const rel of modules){
 const file=new URL(rel,import.meta.url);
 assert.ok(existsSync(file),`Missing module ${rel}`);
 for(const match of readFileSync(file,'utf8').matchAll(/from\s*['"](\.[^'"]+)['"]/g)){
  assert.ok(existsSync(new URL(match[1],file)),`${rel} imports missing ${match[1]}`);
 }
}
assert.ok(existsSync(new URL('../data/living-thesis.json',import.meta.url)),'Missing demo data');
console.log('PASS: demo relationships, evidence provenance, score bounds, null metrics, thresholds, filters, review freshness, deterministic adapter, personal-data separation, unavailable states and page assets.');
