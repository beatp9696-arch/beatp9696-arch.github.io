...
try:
    with sync_playwright() as p:
        ...
        page.locator('#tabbar a[href="smart-money.html"]').click()
        expect(page.locator('.sm-card')).to_have_count(16)
        page.goto(origin+'/portfolio.html?view=research')
        expect(page.locator('.rx-card')).to_have_count(3)
        page.locator('#tabbar a[href="money.html"]').click()
        ...
        page.goto(url+'?book=demo&symbol=COST&thesis=moat')
        expect(page.locator('[data-thesis-tab="moat"]')).to_have_attribute('aria-selected','true')
        expect(page.locator('.lt-pillar')).to_have_count(7)
        ...
        page=mobile.new_page();page.goto(url+'?book=demo&symbol=COST&thesis=earnings')
        expect(page.locator('.lt-earnings')).to_contain_text('$65.9B')
        page.locator('[data-action="refresh"]').click()
        expect(page.locator('.lt-success')).to_contain_text('Demo analysis refreshed')
        mobile.close();browser.close()
        print('PASS: thesis workflows, local persistence, portfolio CRUD, privacy, all tabs and filters, 320–1440px layouts, deep links, route history and errors.')
finally:
