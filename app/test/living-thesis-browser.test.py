...
try:
    with sync_playwright() as p:
        ...
        page.get_by_role('button',name='Save original thesis',exact=True).click()
        page.locator('[data-action="refresh"]').click()
        expect(page.locator('.lt-success')).to_contain_text('Research library reloaded')
        expect(page.locator('.lt-detail-stats')).to_contain_text('INSUFFICIENT DATA')
        page.locator('[data-thesis-tab="timeline"]').click()
        expect(page.locator('.lt-timeline-event')).to_have_count(5)
        ...
        context.close()
        # Every personal holding gets company-specific research, persistent draft adoption and local logos.
        researched=browser.new_context(viewport={'width':1440,'height':1000},service_workers='block',reduced_motion='reduce')
        ...
        page.evaluate("""async symbols=>{const s=await import('/app/js/core/storage.js');s.save('pf.holdings',symbols.map((tk,i)=>({id:'test-'+i,tk,shares:1,cost:1,price:2,sec:'other'})));await s.flushStorage();}""",symbols)
        page.reload();expect(page.locator('.lt-library-strip')).to_be_visible()
        expect(page.locator('[data-holding]')).to_have_count(16)
        portfolio=store(page,'pf.holdings')
        for width in [320,390,768,1440]:
            page.set_viewport_size({'width':width,'height':1000});fitted(page)
        page.locator('.lt-logo img').last.wait_for()
        page.wait_for_function("[...document.querySelectorAll('.lt-logo img')].every(i=>i.complete&&i.naturalWidth>0)")
        assert page.locator('.lt-logo img').count()==16
        page.screenshot(path=str(OUT/'matrices-all-holdings-desktop.png'),full_page=True)
        for symbol in symbols:
            page.locator(f'.lt-company-link[data-symbol="{symbol}"]').click()
            expect(page.locator('.lt-methodology')).to_contain_text('Research draft')
            expect(page.locator('.lt-original')).to_contain_text('Not yet adopted')
            assert page.locator('.lt-original blockquote').inner_text()
            expect(page.locator('.lt-question')).to_have_count(3)
            for tab in ['thesis','moat','timeline','earnings','redteam','sell']:
                page.locator(f'[data-thesis-tab="{tab}"]').click()
                page.set_viewport_size({'width':320,'height':950});fitted(page)
                page.set_viewport_size({'width':1440,'height':1000});fitted(page)
                assert 'Demo analysis' not in page.locator('#lt-panel').inner_text()
                assert 'Mock data' not in page.locator('#lt-panel').inner_text()
                if tab=='moat':expect(page.locator('.lt-pillar')).to_have_count(7)
                if tab=='redteam':expect(page.locator('.lt-red-argument')).to_have_count(5)
                if tab=='sell':expect(page.locator('.lt-condition')).to_have_count(3)
                if tab=='earnings' and symbol=='TSM':expect(page.locator('.lt-earnings')).to_contain_text('NT$')
                if tab=='earnings' and symbol=='BAC':expect(page.locator('.lt-earnings')).to_contain_text('Net interest yield')
            page.locator('.lt-breadcrumb [data-action="back"]').click()
        page.locator('.lt-company-link[data-symbol="MELI"]').click()
        page.set_viewport_size({'width':390,'height':844})
        page.screenshot(path=str(OUT/'matrices-research-mobile.png'),full_page=True)
        draft=page.locator('.lt-original blockquote').inner_text()
        page.locator('[data-action="edit-thesis"]').click()
        expect(page.locator('dialog textarea')).to_have_value(draft)
        page.get_by_role('button',name='Save original thesis',exact=True).click()
        expect(page.locator('.lt-original')).not_to_contain_text('Not yet adopted')
        page.locator('[data-action="resolve"]').first.click()
        page.locator('[data-action="watch"]').nth(1).click()
        page.locator('[data-thesis-tab="sell"]').click()
        page.locator('[data-action="edit-condition"]').first.click()
        page.locator('dialog input[name="title"]').fill('My private boundary')
        page.get_by_role('button',name='Save condition',exact=True).click()
        page.locator('[data-action="refresh"]').click()
        expect(page.locator('.lt-success')).to_contain_text('Research library reloaded')
        expect(page.locator('.lt-condition').first).to_contain_text('My private boundary')
        page.reload();expect(page.locator('.lt-question').first).to_have_class('lt-question is-resolved')
        expect(page.locator('[data-action="watch"]').nth(1)).to_have_attribute('aria-pressed','true')
        page.locator('[data-thesis-tab="thesis"]').click()
        expect(page.locator('.lt-original blockquote')).to_have_text(draft)
        page.locator('[data-action="edit-thesis"]').click()
        page.locator('dialog textarea').fill('My own different reason to invest')
        page.get_by_role('button',name='Save original thesis',exact=True).click()
        expect(page.locator('dialog')).to_have_count(0)
        expect(page.locator('.lt-original blockquote')).to_have_text('My own different reason to invest')
        page.reload();expect(page.locator('.lt-detail-stats')).to_contain_text('INSUFFICIENT DATA')
        expect(page.locator('.lt-original blockquote')).to_have_text('My own different reason to invest')
        page.locator('.lt-evidence summary').first.click()
        source=page.locator('.lt-evidence[open] a').first.get_attribute('href')
        assert source.startswith(origin+'/articles/deep-dive-meli.html#sec-')
        response=page.request.get(source);assert response.ok
        before=store(page,'pf.living.personal.v1')
        researched.route('**/data/living-thesis-library.json',lambda r:r.fulfill(status=503,body='Unavailable'))
        page.locator('[data-action="refresh"]').click()
        expect(page.locator('.lt-alert')).to_be_visible()
        assert store(page,'pf.living.personal.v1')==before
        researched.unroute('**/data/living-thesis-library.json')
        page.locator('[data-action="retry"]').click()
        expect(page.locator('.lt-success')).to_be_visible()
        page.locator('.lt-breadcrumb [data-action="back"]').click()
        ...
        page.reload();expect(page.locator('[data-holding]')).to_contain_text('INSUFFICIENT DATA')
        page.locator('.lt-company-link').click()
        page.locator('[data-thesis-tab="timeline"]').click()
