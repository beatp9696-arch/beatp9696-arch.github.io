...
try:
    with sync_playwright() as p:
        ...
        page.evaluate("""async()=>{const s=await import('./js/core/storage.js');s.save('pf.holdings',[{id:'unknown',tk:'ZZZZ',shares:1,cost:1,price:2}]);await s.flushStorage();}""")
        ...
        page.locator('[data-action="refresh"]').click()
        expect(page.locator('.lt-success')).to_contain_text('Demo analysis refreshed')
        page.evaluate("""async()=>{const s=await import('./js/core/storage.js');const lib=await(await fetch('./data/living-thesis-library.json')).json();s.save('pf.holdings',lib.companies.map((c,i)=>({id:'offline-'+i,tk:c.symbol,shares:1,cost:1,price:2})));await s.flushStorage();}""")
        page.locator('[data-action="personal"]').click()
        expect(page.locator('[data-holding]')).to_have_count(16)
        page.wait_for_function("[...document.querySelectorAll('.lt-logo img')].every(i=>i.complete&&i.naturalWidth>0)")
        assert page.locator('.lt-logo img').count()==16
