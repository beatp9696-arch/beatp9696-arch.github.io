...
def store(page,key): return page.evaluate("async key=>(await import('./js/core/storage.js')).load(key)",key)
def save(page,key,value):
    page.evaluate("async ([k,v])=>{const s=await import('./js/core/storage.js');s.save(k,v);await s.flushStorage();}",[key,value])
def fitted(page):
...
try:
  with sync_playwright() as p:
    ...
    page.goto(origin+'/pp-os/?mode=app');expect(page.locator('.pc-head h1')).to_be_visible()
    ...
    page.goto(origin+'/pp-os/?mode=desktop&open=portfolio&portfolioView=allocation')
    ...
    pending=[]
    delayed.route('**/data/living-thesis-library.json',lambda r:pending.append(r))
    ...
    delayed.close()
    # Packaged cockpit is fully usable offline after first install.
    offline=browser.new_context(viewport={'width':390,'height':844})
    page=offline.new_page();page.goto(url);expect(page.locator('.pc-head h1')).to_be_visible()
    save(page,'pf.holdings',holdings)
    ...
    offline.set_offline(True);page.reload();expect(page.locator('.pf-row')).to_have_count(6)
    expect(page.locator('.pf-total')).to_have_text('$1,000.00');fitted(page)
    offline.close();browser.close()
    print('PASS: cockpit accounting, targets, filters, stock navigation, auto restore, 320–1440px, logo fallback, missing data and offline.')
finally:
