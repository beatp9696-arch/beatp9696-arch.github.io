...
const SYNC_KEYS = new Set([
  ...
  "notes.text",
  "os.name",
  // pf.holdings ไม่อยู่ในนี้โดยตั้งใจ — พอร์ตจริงอยู่เฉพาะเครื่อง ไม่ขึ้น cloud (ดู portfolio.js)
]);
const pick = (obj = {}) => {
  const o = {};
...
// key ที่อยู่ใน meta แต่ไม่อยู่ใน data = tombstone (ถูกลบ ณ เวลานั้น) — การลบจึงแข่งเวลา
// กับการแก้ได้เหมือน value ปกติ ไม่ใช่แพ้เสมอแบบเดิม (เดิม: ลบฝั่งหนึ่ง อีกฝั่ง sync กลับมาคืนศพ)
export function mergeState(local, remote) {
  const data = {};
  const meta = {};
  const pulled = []; // key ที่รับจาก remote (ทั้งค่าใหม่และการลบ)
  let localAhead = false; // มี key ที่ local ใหม่กว่า/remote ไม่มี → ต้อง push กลับ
  const keys = new Set(
    [
      ...Object.keys(local.data || {}),
      ...Object.keys(remote.data || {}),
      ...Object.keys(local.meta || {}),
      ...Object.keys(remote.meta || {}),
  ...
  for (const k of keys) {
    ...
    if (rt > lt) {
      // remote ใหม่กว่า — รับสถานะของมันมาทั้งก้อน: มีค่า = รับค่า, ไม่มีค่า = รับการลบ
      if (inR) data[k] = remote.data[k];
      meta[k] = rt;
      pulled.push(k);
    } else {
      if (inL) {
        ...
        meta[k] = lt; // tombstone ฝั่งเรา — เก็บ timestamp ไว้บอก cloud ว่าลบเมื่อไหร่
      }
      if (lt > rt || (inL && !inR)) localAhead = true;
    }
  }
...
// v2 = meta อาจมี tombstone (key ที่ลบแล้ว) — ฝั่งอ่านต้องเช็ค v ก่อนเชื่อ (ดู syncNow)
function bodyFor(data, meta) {
  return { v: 2, device: deviceName(), at: Date.now(), data, meta };
}
...
export async function connect(token) {
  // จำของเดิมไว้ก่อน — เครื่องที่ sync อยู่แล้วต้องไม่หลุด เพราะพิมพ์ token ใหม่ผิดตัวเดียว
  const prevToken = load("sync.token");
  ...
  try {
    ...
    // token ใหม่ใช้ไม่ได้ → ถอยกลับของเดิม (gistId ไม่ลบ — ไม่ได้เสียหาย และหาใหม่ได้จาก findGist)
    if (prevToken) save("sync.token", prevToken);
    else remove("sync.token");
    setStatus({ state: prevToken ? "on" : "off", busy: false, error: e.message });
...
export async function syncNow() {
  ...
  inflight = (async () => {
    ...
    try {
      ...
      const remoteRaw = (await readRemote(gistId)) ?? { v: 2, data: {}, meta: {} };
      const rdata = pick(remoteRaw.data);
      let rmeta = remoteRaw.meta || {};
      if ((remoteRaw.v ?? 1) < 2) {
        // gist รุ่นเก่า: meta ไม่ได้กรองตาม allowlist และไม่มีแนวคิด tombstone —
        // key ที่มี meta แต่ไม่มี data คือ "ไม่เคย sync" ไม่ใช่ "ถูกลบ" ต้องตัดทิ้ง
        // ไม่งั้น key ที่เพิ่งเข้า allowlist (เช่น money.goals) จะโดนตีความว่าลบแล้วหายทั้งเครื่อง
        rmeta = Object.fromEntries(Object.entries(rmeta).filter(([k]) => k in rdata));
      }
      const remote = { data: rdata, meta: rmeta };
      const merged = mergeState(localState(), remote);
      if (merged.pulled.length) {
        applySync(merged.data, merged.meta);
        // บอก shell ให้วาดแท็บปัจจุบันใหม่ — แอปที่ mount ค้างไว้ถือข้อมูลเก่าใน closure
        // ถ้าไม่รื้อ ผู้ใช้ทั้งไม่เห็นของใหม่ และการแก้ครั้งถัดไปจะ save ของเก่าทับกลับ
        document.dispatchEvent(new CustomEvent("pp-sync-pulled", { detail: merged.pulled }));
      }
