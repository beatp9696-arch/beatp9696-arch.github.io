...
import {loadDemo,analyzeThesis} from './service.js';
...
export function mountWorkspace(body) {
  ...
  let samples=[],request=null;
  const life=new AbortController();
  const scrollHost=()=>body.closest('#shell-view')||body.closest('.win-body')||body;
  const companies=()=>state.scope==='demo'?samples:personalCompanies();
  ...
  function render(focus) {
    ...
    body.innerHTML=`<div class="lt-wrap"><div class="lt-topbar"><a class="lt-wordmark" href="?mode=app&tab=portfolio" data-action="back"><span class="lt-brand-mark" aria-hidden="true"><i></i><i></i><i></i></span>Moatrices<span class="lt-wordmark-sub">PORTFOLIO</span></a><div class="lt-scope" aria-label="Portfolio data source">${button('My portfolio','personal','','',`aria-pressed="${state.scope==='personal'}"`)}${button('Demo portfolio','demo','','',`aria-pressed="${state.scope==='demo'}"`)}</div></div>
      ...
      ${state.loading?`<div class="lt-loading" role="status" aria-label="Loading analysis"><span>${icon('activity')} ${samples.length?'Refreshing demo analysis…':'Loading thesis workspace…'}</span><div class="lt-skeleton"></div><div class="lt-skeleton"></div><div class="lt-skeleton"></div></div>`:''}
    ...
    if(state.view==='allocation') allocation.mount(body.querySelector('.lt-allocation'), {
      onOpenHealth() {
        ...
        route();render();scrollHost().scrollTop=0;
  ...
  function editThesis(c,trigger) {
    ...
    dialog.querySelector('form').addEventListener('submit',async e=>{e.preventDefault();const statement=new FormData(e.target).get('statement').trim();if(!statement){e.target.statement.setCustomValidity('Write your original thesis.');e.target.statement.reportValidity();return;}
      c.thesis.originalStatement=statement;c.originalEdited=c.isDemo?statement!==c.baseline:true;c.reviewedAt=null;
      ...
      if(!await remember(c))return;dialog.close();state.message='Original thesis saved on this device. Reassessment is required.';render('[data-action="edit-thesis"]');
    });
  ...
  function editCondition(c,id,trigger) {
    ...
    const options=[['moatScore','Moat score'],['grossMargin','Gross margin (%)'],['operatingMargin','Operating margin (%)'],['customerGrowth','Customer / volume growth (%)'],['debt','Debt (USD billions)'],['dilution','Share dilution (%)'],['manual','Qualitative condition']];
  ...
  body.addEventListener('click',async e=>{
    ...
    if(action==='personal'||action==='demo'){state.scope=action;save('pf.living.scope',action);state.symbol=null;state.view='health';state.filter='All';state.error='';state.message='';route();render();if(action==='demo'&&!samples.length)await fetchSamples();return;}
    if(['back','health'].includes(action)){state.symbol=null;state.view='health';state.error='';state.message='';route();render();scrollHost().scrollTop=0;return;}
  ...
  route({replace:true});
  if(state.scope==='demo')fetchSamples();else{state.loading=false;render();}
}
