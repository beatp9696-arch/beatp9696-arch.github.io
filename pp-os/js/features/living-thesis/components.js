...
export const logo=c=>`<span class="lt-logo"><b>${esc(c.holding.symbol.slice(0,2))}</b>${['MSFT','V','COST','SNPS','TSM','NVDA','GOOGL'].includes(c.holding.symbol)?`<img src="./assets/brands/${esc(c.holding.symbol)}.png" alt="">`:''}</span>`;
...
export function evidenceRows(c,ids=c.evidence.map(e=>e.id)) {
  ...
  return events.map(e=>`<details class="lt-evidence" data-evidence-id="${esc(e.id)}"><summary><span class="lt-event-dot lt-${tone(e.impact)}"></span><span><b>${esc(e.title)}</b><small>${date(e.publishedAt)} · ${esc(e.sourceLabel)} · ${e.confidence}% confidence</small></span><span class="lt-expand" aria-hidden="true">+</span></summary><div class="lt-evidence-copy"><p>${esc(e.summary)}</p><div class="lt-meta"><span class="lt-${tone(e.impact)}">${esc(e.impact)} thesis</span><span>${esc(pillarName(e.affectedPillar))}</span></div><p class="lt-uncertainty"><b>Still uncertain</b> ${esc(e.uncertainty)}</p><a href="${esc(safeURL(e.sourceUrl))}" target="_blank" rel="noopener noreferrer">Open background source ${icon('arrow-up-right')}</a><small class="lt-source-note">Sample evidence. The linked reference does not substantiate this fictional event.</small></div></details>`).join('');
}
export function safeURL(value) { try {const url=new URL(value);return url.protocol==='https:'?url.href:'';}catch{return '';}}
export function provenance(c,{confidence=c.thesis.confidence,date:stamp=c.holding.lastUpdated,evidenceIds=c.evidence.slice(0,1).map(e=>e.id),uncertainty='This sample has not been verified against current business results.'}={}) {
  return `<div class="lt-provenance"><div class="lt-meta"><span>${number(confidence,'%')} confidence</span><span>${date(stamp)}</span><span>${c.isDemo?'Demo analysis':'Your thesis'}</span></div><p class="lt-uncertainty"><b>Still uncertain</b> ${esc(uncertainty)}</p>${evidenceRows(c,evidenceIds)}</div>`;
}
