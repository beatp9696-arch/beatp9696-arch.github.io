...
try:
    with sync_playwright() as p:
        ...
        expect(page.locator('.rx-earnings-table')).to_be_visible()
        page.goto(origin + '/pp-os/?mode=app&tab=moatrices&view=invalid')
        ...
        failed.route('**/pp-os/data/research.json', lambda r: r.fulfill(status=503, body='Unavailable'))
        page = failed.new_page()
        page.goto(origin + '/index.html')
        expect(page.locator('[data-retry-research]')).to_be_visible()
        failed.unroute('**/pp-os/data/research.json')
